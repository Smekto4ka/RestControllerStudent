...
public class WebController {
    ...
    public String getStudentById(@PathVariable("idStudent") long studentId, Model model) {

        Optional<Student> optionalStudent = studentService.getStudById(studentId);
        try {
        ...
        try {
            ...
            try {
                ...
                if (numberMarks > 0) {
                    ...
                    model.addAttribute("arraysSubject", SubjectEnum.values());
    ...
    public String setMarks(@PathVariable("idStudent") long idStudent,
        ...
        try {
            ...
            if (bindingResult.hasErrors()) {
                ...
                model.addAttribute("student", student);
                model.addAttribute("numberingOfMarks", numberingOfMarks);
                model.addAttribute("arraysPredmet", SubjectEnum.values());
                return "setValue";
            }
            studentService.setMarksByIdStudentsAndSubject(student.getStudentId(), listRating.getPredmet(), listRating.getList());
            return "redirect:/student/" + student.getStudentId();
        } catch (IllegalAccessError ex) {
            System.out.println(ex + "save marks");
            model.addAttribute("warning"
                    , "no Student");
            return "error";
        }
    ...
    public String filterAverageMarks(@Valid @ModelAttribute("filterAverage") FormParametersForFilter
        ...
        model.addAttribute("listStudent", studentService.getStudentsUsingFilter(formParametersForFilter, FilterCriterion.AVERAGE_MARKS));
        return "listIdStudent";
    }
    ...
    public String filterYears(@Valid @ModelAttribute("filterYears") FormParametersForFilter
        ...
        System.out.println(formParametersForFilter);
        model.addAttribute("listStudent", studentService.getStudentsUsingFilter(formParametersForFilter, FilterCriterion.YEARS));
        return "listIdStudent";
    }
    ...
    public String deleteStudent(@PathVariable long idStudent, Model model) {
        if (studentService.deleteStudentById(idStudent))
            return "redirect:/student";
        model.addAttribute("warning"
